...
(function($) {
    ...
    let productsDeleted = 0; // Track number of products deleted
            ...
            success: function(response) {
                if (response && response.success) {
                    if (response.data.is_running) {
                        ...
                        // Store deleted count
                        productsDeleted = response.data.products_processed || 0;
                        ...
                        statusEl.removeClass('success error').show().html(
                            '<p><strong>A deletion process is running!</strong>' + 
                            (response.data.time_elapsed ? ' (' + response.data.time_elapsed + ' ago)' : '') + '</p>' +
                            (productsDeleted > 0 ? '<p>' + productsDeleted + ' products processed so far.</p>' : '') +
                            '<p>Please wait for the current process to complete before starting a new one.</p>' +
                            '<p><a href="javascript:void(0);" class="oh-refresh-status button">Refresh Status</a></p>'
                        );
                        ...
                        if (response.data.has_log) {
                            statusEl.append('<p><button type="button" class="button oh-view-log-btn">View Log</button></p>');
                            bindLogButton();
                        }
                        ...
                        // Start monitoring
                        initStatusMonitoring();
                    } else if (response.data.is_completed) {
                        // Process was completed, show results
                        isProcessRunning = false;
                        productsDeleted = response.data.deleted_count || 0;
                        ...
                        // No process running or completed - hide status area if present
                        isProcessRunning = false;
                        
                        // If there's a status element, hide it (don't show "no process running")
                        let statusEl = $('#oh-process-status');
                        if (statusEl.length > 0 && statusEl.is(':visible')) {
                            statusEl.hide();
                        }
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response && response.success) {
                    ...
                    // Update products deleted count if available
                    if (response.data.products_processed > 0) {
                        productsDeleted = response.data.products_processed;
                    } else if (response.data.deleted_count > 0) {
                        productsDeleted = response.data.deleted_count;
                    }
    ...
    function updateUI(data) {
        ...
        if (data.is_running) {
            ...
            if (data.products_processed > 0) {
                productsDeleted = data.products_processed;
            }
            
            let processingMessage = '<p><strong>Process is running...</strong>' + 
                (data.time_elapsed ? ' (' + data.time_elapsed + ' ago)' : '') + '</p>';
            
            // Add products processed count if available
            if (productsDeleted > 0) {
                processingMessage += '<p>' + productsDeleted + ' products processed so far.</p>';
            }
            
            processingMessage += '<p>You can navigate away from this page. The process will continue in the background.</p>' +
                '<p><a href="javascript:void(0);" class="oh-refresh-status button">Refresh Status</a></p>';
            
            statusEl.removeClass('success error').show().html(processingMessage);
        ...
        // Process completed
            ...
            // Use the deleted count from response
            if (data.deleted_count > 0) {
                productsDeleted = data.deleted_count;
            }
        ...
        // No process running
            ...
            // HIDE the status area - don't show "No process running" message
            statusEl.hide();
    ...
    function toggleLogView() {
        ...
        if (logEl.length === 0) {
            $('<div id="oh-deletion-log"></div>').insertAfter('#oh-process-status');
            logEl = $('#oh-deletion-log');
        }
